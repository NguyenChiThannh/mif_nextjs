...
export async function getRealStatisticsData() {
  try {
    ...
    const userStats =
      await adminStatisticsApi.query.useGetUserStatisticsByYear(currentYear)
    const postStats =
      await adminStatisticsApi.query.useGetPostStatisticsByYear(currentYear)
    const ratingStats =
      await adminStatisticsApi.query.useGetRatingStatisticsByYear(currentYear)
...
const addPdfContent = (doc, reportText) => {
  ...
  doc
    .fontSize(20)
    .text(`Báo cáo tổng kết năm ${currentYear}`, { align: 'center' })
  doc.moveDown()
...
const createSummaryWorksheet = (workbook, reportText) => {
  ...
  worksheet.columns = [
    {
      header: `Báo cáo tổng kết năm ${currentYear}`,
      key: 'content',
      width: 100,
    },
  ]
...
const createChartWorksheet = (workbook, data) => {
  ...
  categories.forEach((category, index) => {
    ...
    // Calculate total
    const total = Object.values(data[category]).reduce(
      (sum, count) => sum + count,
      0,
    )
