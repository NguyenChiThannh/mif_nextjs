...
export default function SentimentAnalysis({ sentimentData }) {
  ...
  const { data: moviesData, isLoading } = movieApi.query.useGetAllMoviesTable(
    currentPage,
    pageSize,
  )
  ...
  const handleExportSentimentCSV = () => {
    const data = {
      ...
      mostPositivePercentage: Number(
        sentimentData.mostPositivePercentage.toFixed(2),
      ),
      mostNegativeMovie: sentimentData.mostNegativeMovie,
      mostNegativePercentage: Number(
        sentimentData.mostNegativePercentage.toFixed(2),
      ),
    ...
    link.setAttribute(
      'download',
      `sentiment_analysis_${new Date().toISOString().split('T')[0]}.csv`,
    )
  ...
  const handleExportSentimentExcel = () => {
    ...
    XLSX.writeFile(
      wb,
      `sentiment_analysis_${new Date().toISOString().split('T')[0]}.xlsx`,
    )
  }
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 gap-4 mt-4'>
              ...
              <div className='space-y-2'>
                ...
                <Tabs
                  ...
                  className='w-full'
                  onValueChange={setSentimentView}
                >
                  <TabsList className='w-full grid grid-cols-2'>
                    <TabsTrigger value='overview'>
                      <div className='flex items-center'>Tổng quan</div>
                    </TabsTrigger>
                    <TabsTrigger value='comparison'>
                      <div className='flex items-center'>So sánh phim</div>
                    </TabsTrigger>
                  </TabsList>
                </Tabs>
              </div>
                ...
                <PaginationContent>
                  ...
                  <PaginationItem className='mt-4 shadow-md rounded-lg'>
                    <PaginationNext
                      ...
                      className={
                        currentPage >= moviesData?.totalPages - 1
                          ? 'pointer-events-none opacity-50'
                          : 'cursor-pointer'
                      }
                    />
