...
export default function MovieDetail({ selectedMovie, setSelectedMovie }) {
  const { isLoading: isLoadingReview, data: review } =
    movieRatingsApi.query.useGetRatingsByMovieId(selectedMovie.id)
  ...
  return (
    ...
    >
      <div className='flex items-center justify-between mb-6'>
        <h3 className='text-lg font-semibold text-foreground'>Chi tiết phim</h3>
        <Button
          variant='outline'
          size='sm'
      ...
      >
        <div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
          ...
          <div className='space-y-2'>
            <p className='text-sm text-muted-foreground'>Số lượng đánh giá</p>
            <p className='font-medium text-foreground'>
              {selectedMovie.ratings?.numberOfRatings || 0}
            </p>
          </div>
          <div className='space-y-2'>
            <p className='text-sm text-muted-foreground'>Điểm trung bình</p>
            <p className='font-medium text-foreground'>
              {selectedMovie.ratings?.averageRating || 0}
            </p>
          </div>
        </div>
      </motion.div>
      ...
      <div className='mt-8'>
        <div className='flex items-center justify-between mb-4'>
          <h3 className='text-lg font-semibold'>
            Chi tiết đánh giá: {selectedMovie.title}
          </h3>
        </div>
                        ...
                        <img
                          ...
                          onError={(e) => {
                            ...
                            e.target.src =
                              ...
                              encodeURIComponent(rating.user.displayName)
                          }}
                        />
                      </div>
                    ...
                    {rating.positiveScore ? (
                      <>
                        ...
                        <span className='text-xs mt-1 block'>
                          {(rating.positiveScore * 100).toFixed(1)}%
                        </span>
                      </>
                    ) : (
                  ...
                  <td className='p-3 text-center min-w-[120px]'>
                    {rating.negativeScore ? (
                      <>
                        ...
                        <span className='text-xs mt-1 block'>
                          {(rating.negativeScore * 100).toFixed(1)}%
                        </span>
                      </>
                    ) : (
                      <span className='text-xs text-gray-500'>N/A</span>
                  ...
                  <td className='p-3 text-center min-w-[120px]'>
                    {rating.neutralScore ? (
                      <>
                        ...
                        <span className='text-xs mt-1 block'>
                          {(rating.neutralScore * 100).toFixed(1)}%
                        </span>
                      </>
                    ) : (
                      <span className='text-xs text-gray-500'>N/A</span>
                  ...
                  <td className='p-3 text-center min-w-[120px]'>
                    {rating.mixedScore ? (
                      <>
                        ...
                        <span className='text-xs mt-1 block'>
                          {(rating.mixedScore * 100).toFixed(1)}%
                        </span>
                      </>
                    ) : (
                      <span className='text-xs text-gray-500'>N/A</span>
        ...
        {review?.content.length === 0 && (
          <div className='text-center p-8 bg-accent/10 rounded-lg'>
            <p className='text-muted-foreground'>
              Không có dữ liệu đánh giá chi tiết cho phim này.
            </p>
          </div>
        )}
        ...
        <div className='mt-4 p-4 bg-accent/10 rounded-lg'>
          ...
          <p className='text-sm text-muted-foreground'>
            Bảng trên hiển thị chi tiết từng đánh giá của người dùng về phim
            &quot;{selectedMovie.title}&quot;. Mỗi đánh giá được hiển thị kèm
            theo điểm số và phân tích cảm xúc (nếu có). Các đánh giá được phân
            tích thành các điểm số cảm xúc tích cực, tiêu cực, trung tính và hỗn
            hợp.
          </p>
        </div>
      </div>
