    ...
    return (
        <Dialog open={isOpen} onOpenChange={onClose}>
            <DialogContent className="max-w-lg rounded-xl border border-muted shadow-lg bg-background">
                <DialogHeader>
                    <DialogTitle className="text-lg font-semibold text-foreground">User Details</DialogTitle>
                </DialogHeader>
                <div className="space-y-3">
                    {[
                        ...
                        { label: 'Role', value: userData?.authorities?.[userData?.authorities.length - 1]?.authority },
                        { label: 'Created At', value: formatDate(userData?.createdAt) },
                        { label: 'Updated At', value: formatDate(userData?.updatedAt) },
                    ].map((item, index) => (
                        <div key={index} className="flex justify-between px-4 py-2 rounded-md bg-muted/50">
