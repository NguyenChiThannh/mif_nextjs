...
export default function Users() {
  ...
  const { isLoading: isLoadingUsers, data: usersData } =
    userApi.query.useGetAllUsersTable(currentPage, pageSize)
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        header: 'Status',
        cell: ({ row }) =>
          row.original.accountNonLocked ? 'Unlocked' : 'Locked',
      },
      {
        ...
        cell: ({ row }) =>
          row
            .getValue('authorities')
            ?.[
              row.getValue('authorities').length - 1
                ...
                <DropdownMenuItem
                  onClick={() =>
                    row.original.accountNonLocked
                      ? handleBlockUser(row.original.id, true)
                      : handleBlockUser(row.original.id, false)
                  }
                >
