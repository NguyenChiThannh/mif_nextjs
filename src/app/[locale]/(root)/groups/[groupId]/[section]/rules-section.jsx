...
export default function RulesSection({ isOwner, group, t }) {
  ...
  const { data: rules, isLoading } = groupRulesApi.query.useGetRulesByGroupId(
    group.id,
  )
  const deleteRuleMutation = groupRulesApi.mutation.useDeleteRuleFromGroup(
    group.id,
  )
  ...
  return (
    ...
    >
      <Card className='w-full max-w-3xl mx-auto my-8 border-border bg-card'>
        <CardContent>
          {/* Title */}
          <motion.div variants={itemVariants} className='my-6 grid gap-2'>
            <div className='flex justify-between items-center'>
              <p className='font-bold text-lg text-foreground flex items-center'>
                {t('rules')}
              </p>
              {isOwner && (
                <motion.div
