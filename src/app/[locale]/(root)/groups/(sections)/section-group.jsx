...
export function SectionGroup({ movieCategories, userId, t }) {
  ...
  const { isLoading: loadingOwnerGroups, data: ownerGroups } =
    groupsApi.query.useFindByOwnerId(0, 24)
  const { isLoading: loadingUserGroups, data: userGroups } =
    groupsApi.query.useGetUserGroups(0, 24, userId)
          ...
          <>
            ...
            {activeTab === 'all' && (
              <>
                {Array.from(
                  new Map(
                    [...ownerGroups.content, ...userGroups.content].map(
