...
export const BADGE_THRESHOLDS = [
  ...
  },
  {
    level: 'GOLD',
    color:
      'bg-gradient-to-r from-yellow-300 to-amber-400 text-amber-900 border-2 border-yellow-500 shadow-inner',
  },
  {
    level: 'SILVER',
    color:
...
export default function NotificationItem({ notification, onClick }) {
  ...
  const handleReadNotification = () => {
    ...
    switch (notification.type) {
      ...
      case 'COMMENT':
        router.push(
          `/groups/${notification.groupId}/post/${notification.groupPostId}`,
        )
        break
      case 'EVENT':
        router.push(
          notification.url || `/groups/${notification.groupId}/events`,
        )
        break
      case 'ACCEPT_REQUEST':
      case 'JOIN_REQUEST':
      ...
      case 'BADGE_EARNED':
        router.push(
          `/badge?level=${badgeLevel}&groupId=${notification.groupId}`,
        )
        break
      case 'POST_BLOCKED':
