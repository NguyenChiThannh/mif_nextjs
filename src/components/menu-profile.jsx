      ...
      <DropdownMenu modal={false}>
        <DropdownMenuTrigger asChild>
          <Avatar className='w-8 h-8 flex items-center justify-center object-contain'>
            ...
            <AvatarFallback className='flex items-center justify-center uppercase'>
              {data?.displayName && data?.displayName[0]}
            </AvatarFallback>
          </Avatar>
        </DropdownMenuTrigger>
        <DropdownMenuContent className='w-56 overflow-hidden'>
          ...
          {(role === 'ADMIN' || role === 'CONTENT_CREATOR') && goToAdmin && (
            <DropdownMenuItem>
              ...
              <Link href={'/admin/dashboard'}>{t('admin_page')}</Link>
            </DropdownMenuItem>
          )}
          {goToHome && (
            <DropdownMenuItem>
              <House className='mr-2 h-4 w-4' />
              <Link href={'/'}>{t('home_page')}</Link>
            </DropdownMenuItem>
          )}
          {!admin && (
            <>
              <DropdownMenuGroup>
                <DropdownMenuItem>
                  <Link href={`/user/${id}`} className='w-full h-full flex'>
