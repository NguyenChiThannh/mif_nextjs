...
export default function CardMember({
  ...
  const acceptInvitationMutation =
    groupsApi.mutation.useAcceptInvitation(groupId)
  const rejectInvationMutation = groupsApi.mutation.useRejectInvitation(groupId)
  const removeMemberFromGroupMutation =
    groupsApi.mutation.useRemoveMemberFromGroup(groupId)
  ...
  return (
    <div className='bg-background rounded-lg shadow-md p-4 flex items-center justify-between'>
      <div className='flex items-center gap-4'>
        <Avatar>
          <AvatarImage
            src={member?.avatar || member?.profilePictureUrl}
        ...
        <div>
          <Link className='flex' href={`/user/${member?.id}`}>
            <h3 className='font-bold hover:underline'>{member?.displayName}</h3>
          </Link>
          {type === 'invitation' || cardOwner || (
            <p className='text-muted-foreground text-xs font-bold'>
              {t('joined_a_while_ago')} {calculateTimeAgo(date, locale)}
            </p>
          )}
        </div>
      </div>
