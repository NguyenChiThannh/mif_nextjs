...
const addPendingInvitation = async (data) => {
  const res = await privateApi.post(
    `/groups/${data.groupId}/pending-invitations`,
  )
  return res.data
}
...
const acceptInvitation = async (data) => {
  const res = await privateApi.post(
    `/groups/${data.groupId}/accept-invitations/${data.userId}`,
  )
  return res.data
}
...
const removePendingInvitation = async (data) => {
  const res = await privateApi.delete(
    `/groups/${data.groupId}/pending-invitations/${data.userId}`,
  )
  return res.data
}
      ...
      return useMutation({
        ...
        onSuccess: () => {
          ...
          queryClient.invalidateQueries({
            queryKey: QUERY_KEY.groupsByOwnerId(0, 24),
          })
          queryClient.invalidateQueries({
            queryKey: QUERY_KEY.userGroups(0, 24),
          })
        },
      })
    },
