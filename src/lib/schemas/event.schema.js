...
export const schemaEvent = z.object({
    eventName: z.string().min(2, 'Tên sự kiện phải có ít nhất 2 ký tự').max(100, 'Tên sự kiện tối đa 100 ký tự'),
    ...
    startDate: z.any().refine(
        (val) => {
            ...
            if (typeof val === 'object' && val.year && val.month && val.day) {
                const date = new Date(Date.UTC(val.year, val.month - 1, val.day, val.hour || 0, val.minute || 0, val.second || 0, val.millisecond || 0));
    ...
    eventType: z.enum(["OFFLINE", "ONLINE"], { required_error: 'Hình thức sự kiện là bắt buộc' }),
    eventPicture: z.string(),
    socialType: z.enum(["MIF_LIVE", "OTHER"], { required_error: 'Nền tảng là bắt buộc' }),
    link: z.string().url('Link phải là một URL hợp lệ').optional(),
