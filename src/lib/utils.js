...
export const parseHtmlWithClasses = (domNode) => {
  if (domNode && domNode.name === 'h1') {
    return (
      <h1
        {...domNode.attribs}
        className={`text-3xl ${domNode.attribs.class || ''}`}
      >
  ...
  if (domNode && domNode.name === 'h2') {
    return (
      <h2
        {...domNode.attribs}
        className={`text-2xl ${domNode.attribs.class || ''}`}
      >
        {domToReact(domNode.children)}
      </h2>
    )
  ...
  if (domNode && domNode.name === 'h3') {
    return (
      <h2
        {...domNode.attribs}
        className={`text-xl ${domNode.attribs.class || ''}`}
      >
        {domToReact(domNode.children)}
      </h2>
    )
  ...
  if (domNode && domNode.name === 'a') {
    return (
      <>
        <Link href={domNode.attribs.href}>{domToReact(domNode.children)}</Link>
      </>
    )
  }
  return undefined
